...
use crate::{
    ...
    runtime::{
        ...
        pairing_code::{PairingCodeRejectionReason, PairingCodeRequest, PairingCodeResponse},
        pairing_sessions::{
    ...
    Shutdown: Future<Output = ShutdownReason> + Send,
{
    ...
    node.kademlia_membership_records_key = node.discovery.kademlia.then(|| {
    ...
    let mut kademlia_membership_records_key = node.kademlia_membership_records_key.clone();
    let mut kademlia_membership_record_lookup_keys = kademlia_membership_record_lookup_keys(
        &node.network_name,
        node.membership_tag.as_deref(),
        ...
        tokio::select! {
            ...
            _ = timers.code_pairing.tick() => {
                ...
                if let Err(error) = persist_code_pairing_sessions(
                    ...
                    log_pairing_persistence_failure("periodic_checkpoint", &error);
                }
            }
...
struct RuntimeTimers {
    ...
    code_pairing: tokio::time::Interval,
}

fn load_code_pairing_sessions(
    store: Option<&PairingStateStore>,
    network_name: &str,
    now_unix_seconds: u64,
    now: Instant,
) -> Result<CodePairingSessions, RunnerError> {
    let Some(store) = store else {
        return Ok(CodePairingSessions::new());
    };
    let Some(bytes) = store.load()? else {
        return Ok(CodePairingSessions::new());
    };
    Ok(CodePairingSessions::restore_persisted(
        &bytes,
        network_name,
        now_unix_seconds,
        now,
...
fn persist_code_pairing_sessions(
    store: Option<&PairingStateStore>,
...
fn log_pairing_persistence_failure(phase: &str, error: &RunnerError) {
    log_runtime_event(
        LogLevel::Error,
        "pairing_state_persist_failed",
        &[("phase", phase), ("reason", &format!("{error:?}"))],
    );
}

fn reconcile_persisted_pairing_enrollments(
...
fn reconcile_persisted_pairing_enrollments_with(
    ...
    let prepared = enrollments
        ...
        .cloned()
        .collect::<Vec<_>>();
    let network_name = forwarder.config().network.name.clone();
    for enrollment in &prepared {
        sessions.validate_prepared_recovery(&network_name, enrollment)?;
    }
    ...
    for enrollment in &enrollments {
        ...
        if &identity.peer_id != expected_local {
            return Err(crate::pairing::PairingError::LocalIdentityMismatch {
    ...
    *membership = next_membership;
    ...
    for enrollment in &prepared {
        match enrollment.role {
            PairingEnrollmentRole::Inviter => sessions
                .recover_prepared_open(&network_name, enrollment)
                .map(|actions| {
    ...
    log_runtime_event(
        LogLevel::Info,
        "pairing_enrollments_reconciled",
        &[
...
fn stop_code_pairing_provider(swarm: &mut Swarm<Behaviour>, locator: Option<&str>) {
    ...
    swarm
        .behaviour_mut()
        .kad
        .stop_providing(&kademlia_pairing_code_key(locator));
}

fn drive_code_pairing_discovery(
    swarm: &mut Swarm<Behaviour>,
    sessions: &mut CodePairingSessions,
    identity: &NodeIdentity,
    network_name: &str,
...
fn send_pairing_code_hello(
    ...
    let Some((operation_id, code)) = sessions
        .active_join_code()
        .map(|(operation_id, code)| (operation_id.to_owned(), code.clone()))
    else {
        return;
    };
...
fn handle_pair_rpc_request(
    ...
    let result = match request {
        ...
        PairRpcRequest::PairJoin {
            ...
            let parsed_code = code.parse().map_err(|error| {
                pair_rpc_error(
                    PairRpcErrorCode::InvalidRequest,
                    format!("invalid pairing code: {error:?}"),
                    false,
                )
            });
            parsed_code.and_then(|code| {
                if let Some(vpn_ip) = requested_vpn_ip.as_deref()
                    && vpn_ip.parse::<IpAddr>().is_err()
                {
                    return Err(pair_rpc_error(
                        PairRpcErrorCode::InvalidRequest,
                        "requested VPN IP is invalid",
                        false,
                    ));
                }
                let routes = pair_rpc_routes_to_config(requested_routes.unwrap_or_default())?;
                ...
                persist_code_pairing_sessions(store, sessions, &network_name)
                    .map_err(runner_error_pair_rpc)?;
                Ok(PairRpcResult::JoinStarted(PairRpcJoinStarted {
                    operation_id: started.operation_id,
                    network_name: network_name.clone(),
                    local_peer: local_peer.clone(),
                    expires_at_unix_seconds: started.expires_at_unix_seconds,
        ...
        PairRpcRequest::PairStatus { operation_id } => {
            pairing_rpc_status(sessions, &operation_id, &network_name, &local_peer)
                .map(|status| PairRpcResult::OperationStatus(Box::new(status)))
        }
        PairRpcRequest::PairApprove {
                ...
                && vpn_ip.parse::<IpAddr>().is_err()
            {
                return Err(pair_rpc_error(
                    PairRpcErrorCode::InvalidRequest,
                    "assigned VPN IP is invalid",
                    false,
                ));
            }
            let granted_routes = pair_rpc_routes_to_config(granted_routes)?;
            let approval = sessions
            ...
            let (offer, response) = if let Some(enrollment) = sessions.enrollment(&operation_id) {
                    ...
                    || enrollment.approval_id.as_deref() != Some(approval_id.as_str())
                {
                    return Err(pair_rpc_error(
                        ...
                        "pairing enrollment conflicts with this approval",
                        false,
                    ));
                }
                let Some(offer) = enrollment.offer.clone() else {
                    return Err(pair_rpc_error(
                        PairRpcErrorCode::InvalidState,
                        "prepared inviter enrollment is missing its offer",
                        false,
                    ));
                };
                ...
                .map_err(|error| {
                    pair_rpc_error(
                        PairRpcErrorCode::InvalidRequest,
                        format!("pairing grant is invalid: {error:?}"),
                        false,
                    )
                })?
            };
            let prepared =
                prepare_pairing_runtime_enrollment(forwarder, &offer, &response, identity, now)
                    .map_err(runner_error_pair_rpc)?;
            sessions
                .prepare_enrollment(
                    &network_name,
        ...
        PairRpcRequest::PairReject {
            ...
            .reject(&operation_id, &approval_id)
            .map_err(code_session_pair_rpc)
            .and_then(|actions| {
                stop_code_pairing_provider(swarm, actions.stop_providing_locator.as_deref());
                persist_code_pairing_sessions(store, sessions, &network_name)
        ...
        PairRpcRequest::PairCancel { operation_id } => sessions
            .cancel(&operation_id)
            .map_err(code_session_pair_rpc)
            .and_then(|actions| {
                stop_code_pairing_provider(swarm, actions.stop_providing_locator.as_deref());
...
fn pairing_rpc_status(
    ...
    network_name: &str,
    local_peer: &str,
) -> Result<PairRpcOperationStatus, PairRpcResponseEnvelope> {
    let expires_at_unix_seconds = sessions.operation_expires_at(operation_id).ok_or_else(|| {
        pair_rpc_error(
            PairRpcErrorCode::NotFound,
            "pairing operation was not found",
            false,
        )
    })?;
    if let Ok(status) = sessions.open_status(operation_id) {
        let (phase, discovery, candidate, artifacts_ready, failure) = match status {
            PairingOpenStatus::Searching { discovery, .. } => (
                ...
                Some(pair_rpc_discovery_stage(discovery)),
                None,
                false,
                None,
            ),
                ...
                let fingerprint = sessions
                    ...
                    .map(|approval| {
                        URL_SAFE_NO_PAD.encode(Sha256::digest(
                            approval.request.payload.joiner_public_key.as_bytes(),
                        ))
                    })
                    .unwrap_or_default();
                (
            ...
            PairingOpenStatus::Rejected => (
                ...
                None,
                false,
                Some(pair_rpc_failure(
                    PairRpcFailureCode::Rejected,
                    "pairing rejected",
                    false,
                )),
            ),
            PairingOpenStatus::Cancelled => (PairRpcPhase::Cancelled, None, None, false, None),
            PairingOpenStatus::Expired => (
                ...
                false,
                Some(pair_rpc_failure(
                    PairRpcFailureCode::Expired,
                    "pairing expired",
                    false,
                )),
            ),
            PairingOpenStatus::Failed { reason } => (
                PairRpcPhase::Failed,
                None,
                None,
                false,
                Some(pair_rpc_failure(
                    PairRpcFailureCode::Internal,
                    reason,
                    false,
                )),
            ),
    ...
    let status = sessions
        .join_status(operation_id)
        .map_err(code_session_pair_rpc)?;
    let (phase, discovery, failure, artifacts_ready) = match status {
        PairingJoinStatus::Searching { discovery, .. } => (
            PairRpcPhase::Discovering,
            Some(pair_rpc_discovery_stage(discovery)),
            None,
            false,
        ...
        PairingJoinStatus::Failed { reason } => (
            PairRpcPhase::Failed,
            None,
            Some(pair_rpc_failure(
                PairRpcFailureCode::Protocol,
                reason,
                false,
            )),
            false,
...
fn pair_rpc_discovery_stage(stage: PairingDiscoveryStage) -> PairRpcDiscoveryStage {
    match stage {
        PairingDiscoveryStage::Lan => PairRpcDiscoveryStage::Lan,
        PairingDiscoveryStage::Public => PairRpcDiscoveryStage::PublicDht,
    }
}

fn pair_rpc_failure(
    code: PairRpcFailureCode,
...
fn pair_rpc_routes_to_config(
    routes: Vec<PairRpcRoute>,
) -> Result<Vec<RouteConfig>, PairRpcResponseEnvelope> {
    let routes = routes
        .into_iter()
        ...
        .collect::<Vec<_>>();
    for route in &routes {
        route.prefix().map_err(|error| {
            pair_rpc_error(
                PairRpcErrorCode::InvalidRequest,
                format!("invalid route {}: {error:?}", route.prefix),
                false,
            )
        })?;
    }
...
fn config_routes_to_pair_rpc(routes: Vec<RouteConfig>) -> Vec<PairRpcRoute> {
    routes
        .into_iter()
...
fn code_session_pair_rpc(error: CodePairingSessionError) -> PairRpcResponseEnvelope {
    let (code, retryable) = match error {
        CodePairingSessionError::NotFound => (PairRpcErrorCode::NotFound, false),
        CodePairingSessionError::Busy | CodePairingSessionError::Capacity => {
            (PairRpcErrorCode::Busy, true)
        }
        CodePairingSessionError::Conflict => (PairRpcErrorCode::Conflict, false),
        CodePairingSessionError::NotAwaitingApproval
        | CodePairingSessionError::ApprovalMismatch => (PairRpcErrorCode::InvalidState, false),
        CodePairingSessionError::InvalidExpiry
        | CodePairingSessionError::InvalidOperationId
        | CodePairingSessionError::InvalidTicket
        | CodePairingSessionError::InvalidCode(_) => (PairRpcErrorCode::InvalidRequest, false),
        CodePairingSessionError::ExpiryOverflow
        | CodePairingSessionError::Serialization(_)
        | CodePairingSessionError::PersistedNetworkMismatch { .. }
        | CodePairingSessionError::InvalidPersistedState(_) => (PairRpcErrorCode::Internal, false),
    };
    pair_rpc_error(code, error.to_string(), retryable)
}

fn runner_error_pair_rpc(error: RunnerError) -> PairRpcResponseEnvelope {
    pair_rpc_error(PairRpcErrorCode::Internal, format!("{error:?}"), false)
}

fn pair_rpc_error(
    code: PairRpcErrorCode,
    message: impl Into<String>,
    retryable: bool,
) -> PairRpcResponseEnvelope {
...
fn handle_runtime_control_request(
    ...
    match request {
        ...
        RuntimeControlRequest::PairRpc { respond_to, .. } => {
            if respond_to
                .send(
                    crate::runtime::control_socket::PairRpcResponseEnvelope::error(
                        crate::runtime::control_socket::PairRpcErrorCode::Unavailable,
                        "pairing control is not initialized",
                        true,
                    ),
                )
                .is_err()
...
fn extend_runtime_discovery_summary_lines(lines: &mut Vec<String>, snapshot: &RuntimeSnapshot) {
    lines.extend([
        ...
        format!(
            ...
            snapshot.relay_outbound_circuits_established
        ),
        format!(
            "relay_inbound_circuits_established {}",
            snapshot.relay_inbound_circuits_established
        ),
    ]);
}
...
fn kademlia_peer_address_is_advertisable(address: &Multiaddr) -> bool {
        ...
        .any(|protocol| matches!(protocol, Protocol::P2pCircuit))
    {
        return supports_relayed_peer_dial_transport(address)
            && relayed_address_uses_public_relay_transport(address);
    }
...
#[allow(clippy::too_many_arguments)]
fn reconcile_runtime_kademlia_scope(
    node: &mut P2pNode,
    local_capabilities: &ControlCapabilities,
    previous_membership_tags: &[String],
    rendezvous_key: &mut Option<kad::RecordKey>,
    lookup_keys: &mut Vec<kad::RecordKey>,
    membership_records_key: &mut Option<kad::RecordKey>,
    membership_record_lookup_keys: &mut Vec<kad::RecordKey>,
) {
    if node.membership_tag == local_capabilities.membership_tag {
        return;
    }
    ...
    node.membership_tag = local_capabilities.membership_tag.clone();
    *rendezvous_key = node.discovery.kademlia.then(|| {
        crate::runtime::p2p::kademlia_rendezvous_key(
            &node.network_name,
            node.membership_tag.as_deref(),
        )
    });
    *membership_records_key = node.discovery.kademlia.then(|| {
        crate::runtime::p2p::kademlia_membership_records_key(
            &node.network_name,
            node.membership_tag.as_deref(),
        )
    ...
    *membership_record_lookup_keys = kademlia_membership_record_lookup_keys(
        &node.network_name,
        node.membership_tag.as_deref(),
        membership_records_key.as_ref(),
        previous_membership_tags,
    );
    node.kademlia_rendezvous_key.clone_from(rendezvous_key);
    node.kademlia_membership_records_key
        .clone_from(membership_records_key);
    log_runtime_event(
        LogLevel::Info,
        "runtime_membership_scope_reconfigured",
        &[(
            "membership_scoped",
            &node.membership_tag.is_some().to_string(),
        )],
    );
}
...
impl PublicDiscoveryBackoff {
    ...
    fn record_outgoing_error(
        ...
        if !self.bootstrap_peers.contains(&peer)
            || !dial_error_reports_public_bootstrap_failure(error)
        {
            return None;
        }
        if self.suppresses(now) {
            return None;
        }
...
impl AutoRelayState {
    ...
    fn next_reservation_targets(&mut self, now: Instant) -> Vec<(Libp2pPeerId, Multiaddr)> {
        if self.policy.max_reservations == 0
...
fn admit_kademlia_relay_infrastructure_peer<'a>(
    ...
    if !infrastructure_peers.can_admit(peer) || !auto_relay.record_candidate(peer, address.clone())
    {
        return;
    }
...
fn connected_auto_relay_candidate_address(
    ...
    auto_relay_candidate_address(peer, address)
}
...
fn remove_unconfirmed_infrastructure_peer(
    infrastructure_peers: &mut InfrastructurePeers,
    auto_relay: &mut AutoRelayState,
    peer: Libp2pPeerId,
    reason: &str,
...
fn best_auto_relay_candidate_address<'a>(
    peer: Libp2pPeerId,
...
fn ranked_auto_relay_candidate_addresses<'a>(
    peer: Libp2pPeerId,
    addresses: impl IntoIterator<Item = &'a Multiaddr>,
    local_networks: &[LocalInterfaceNetwork],
) -> Vec<Multiaddr> {
    ...
    candidates.sort_by_key(|address| auto_relay_candidate_address_rank(address, local_networks));
    candidates
}

fn auto_relay_candidate_address_rank(
    address: &Multiaddr,
    local_networks: &[LocalInterfaceNetwork],
) -> u8 {
    let Some(ip) = first_ip_in_multiaddr(address) else {
        return 3;
    };
    if local_networks.iter().any(|network| network.contains(ip)) {
        return 0;
    }
    if !private_ip_address(ip) && direct_libp2p_address_ip_is_advertisable(ip) {
        return 1;
    }
    2
}
...
fn relayed_address_uses_public_relay_transport(address: &Multiaddr) -> bool {
    let mut relay_transport = Multiaddr::empty();
    for protocol in address {
        if matches!(protocol, Protocol::P2pCircuit) {
            return relay_transport_is_publicly_dialable(&relay_transport);
        }
        relay_transport.push(protocol);
    }
...
impl RelayReadiness {
    ...
    fn relay_ready(&self, relay: Libp2pPeerId) -> bool {
        self.accepted_reservations.contains(&relay)
            && self.relayed_listen_addresses.contains_key(&relay)
    }

    fn ready_relays(&self) -> Vec<Libp2pPeerId> {
        self.accepted_reservations
            .iter()
            .filter(|relay| self.relayed_listen_addresses.contains_key(relay))
            .copied()
            .collect()
    }
...
fn dial_relay_ready_configured_peers(
    ...
    let mut available_relay_addresses = relay_addresses.to_vec();
    for address in relay_readiness.ready_relay_addresses(relay) {
        if !available_relay_addresses.contains(&address) {
            available_relay_addresses.push(address);
        }
    }
    sort_relay_addresses_by_local_reachability(
        &mut available_relay_addresses,
...
fn relay_ready_configured_peer_targets(
    ...
    let mut addresses = Vec::new();
    let configured_peers = configured_peers.into_iter().collect::<Vec<_>>();
    let configured_peer_set = configured_peers.iter().copied().collect::<HashSet<_>>();
    ...
    let relay_reservation_addresses = relay_addresses
        .iter()
        .filter(|(peer, _)| *peer == relay)
...
fn sort_relay_addresses_by_local_reachability(
    addresses: &mut [(Libp2pPeerId, Multiaddr)],
    local_networks: &[LocalInterfaceNetwork],
) {
    addresses
        .sort_by_key(|(_, address)| auto_relay_candidate_address_rank(address, local_networks));
}
...
fn dial_error_reports_public_bootstrap_failure(error: &DialError) -> bool {
    dial_error_reports_public_no_route(error)
        || matches!(
            error,
            DialError::Transport(errors)
                if !errors.is_empty()
                    && errors
                        .iter()
                        .all(|(_, error)| matches!(error, TransportError::Other(_)))
        )
}
...
async fn handle_swarm_event(
    ...
    match event {
        ...
        SwarmEvent::Behaviour(BehaviourEvent::PairingCode(event)) => {
            handle_pairing_code_event(swarm, &mut context, event)?;
        }
        ...
        SwarmEvent::Behaviour(event) => {
            ...
            let mut behaviour_context = BehaviourEventContext {
                ...
                auto_relay: context.auto_relay,
                paths: context.paths,
                relay_addresses: context.relay_addresses,
                configured_peer_addresses: context.configured_peer_addresses,
                relay_server_enabled: context.relay_server_enabled,
...
fn advertise_relay_server_listener_address(
    swarm: &mut Swarm<Behaviour>,
    relay_server_enabled: bool,
    endpoint: &ConnectedPoint,
) {
...
fn handle_pairing_code_request(
    ...
    match request {
        PairingCodeRequest::Hello { hello } => {
            let Some((operation_id, code, expires_at_unix_seconds)) = context
                .code_pairing_sessions
                .active_open_code_for_locator(&hello.payload.locator)
                .map(|(operation_id, code, expires)| {
                    (operation_id.to_owned(), code.clone(), expires)
                })
            else {
            ...
            match answer_pairing_code_hello_at(
                ...
                Ok((challenge, session)) => {
                    ...
                    send_pairing_code_response(
                        swarm,
                        channel,
                        PairingCodeResponse::Challenge {
                            challenge: Box::new(challenge),
                        },
                    )
                }
                Err(_) => send_pairing_code_unavailable(swarm, channel),
            }
        }
        PairingCodeRequest::Submit { request } => {
            let now = current_unix_seconds_lossy();
            ...
            let Some(inbound) = context
                .code_pairing_sessions
                .take_inbound_session(peer, &request.payload.rendezvous_token)
            else {
                return send_pairing_code_unavailable(swarm, channel);
            };
            ...
            let approval = PendingApproval::new(
                ...
                peer,
                inbound.session.expires_at_unix_seconds(),
                *request,
            )?;
            ...
            if let Err(error) = persist_code_pairing_sessions(
                ...
                log_pairing_persistence_failure("inbound_approval", &error);
                if let Ok(actions) = context.code_pairing_sessions.cancel(&operation_id) {
                    stop_code_pairing_provider(swarm, actions.stop_providing_locator.as_deref());
                }
                if let Err(error) = persist_code_pairing_sessions(
                    context.pairing_state_store,
                    ...
                    log_pairing_persistence_failure("inbound_approval_fail_closed", &error);
                }
                return send_pairing_code_unavailable(swarm, channel);
            }
            send_pairing_code_response(swarm, channel, response)
        }
        PairingCodeRequest::Poll { ticket } => send_pairing_code_response(
            swarm,
            channel,
            context.code_pairing_sessions.poll_response(
                peer,
                &ticket,
                current_unix_seconds_lossy(),
            ),
...
fn handle_pairing_code_response(
    swarm: &mut Swarm<Behaviour>,
    context: &mut SwarmEventContext<'_>,
    peer: Libp2pPeerId,
    request_id: request_response::OutboundRequestId,
    ...
    let Some(pending) = context
        ...
        log_runtime_event(
            LogLevel::Warn,
            "pairing_code_unexpected_response",
            &[("peer", &peer.to_string()), ("reason", "unknown_request")],
        );
        return Ok(());
    };
    let expected_peer = match &pending {
        OutboundCodeRequest::Hello(outbound) => outbound.peer,
    ...
    if expected_peer != peer {
        ...
        log_runtime_event(
            ...
            &[
                ("peer", &peer.to_string()),
                ("reason", "transport_peer_mismatch"),
            ],
        );
        return Ok(());
    }
    ...
    match (pending, response) {
        (OutboundCodeRequest::Hello(outbound), PairingCodeResponse::Challenge { challenge }) => {
            ...
            let Some((requested_vpn_ip, requested_routes)) = context
                .code_pairing_sessions
                .join_request_options(&outbound.operation_id)
            else {
                return Ok(());
            };
        ...
        (
            ...
            if let Err(error) = context.code_pairing_sessions.set_remote_pending(
                ...
                fail_code_pairing_join(
                    context,
                    &outbound.operation_id,
                    "pairing peer returned an invalid approval ticket",
                )?;
                log_runtime_event(
                    LogLevel::Warn,
                    "pairing_code_pending_rejected",
                    &[("peer", &peer.to_string()), ("reason", &error.to_string())],
                );
            } else if let Err(error) = persist_code_pairing_sessions(
                context.pairing_state_store,
                context.code_pairing_sessions,
                &context.local_capabilities.network_name,
            ) {
        ...
        (
            OutboundCodeRequest::Submit(outbound) | OutboundCodeRequest::Poll(outbound),
            PairingCodeResponse::Accepted { response },
        ) => {
            if outbound.offer.payload.inviter_peer != peer.to_string() {
                fail_code_pairing_join(
                    context,
                    &outbound.operation_id,
                    "pairing response transport identity mismatch",
                )?;
                return Ok(());
            }
            let now = current_unix_seconds_lossy();
            if let Err(error) = response.verify_for_offer_at(&outbound.offer, context.identity, now)
            {
                fail_code_pairing_join(
                    context,
                    &outbound.operation_id,
                    "pairing peer returned an invalid acceptance",
                )?;
                log_runtime_event(
                    LogLevel::Warn,
                    "pairing_code_acceptance_rejected",
                    &[
                        ("peer", &peer.to_string()),
                        ("reason", &format!("{error:?}")),
                    ],
            ...
            let prepared = match prepare_pairing_runtime_enrollment(
                ...
                Err(error) => {
                    fail_code_pairing_join(
                        context,
                        &outbound.operation_id,
                        "pairing acceptance conflicts with the running network configuration",
                    )?;
                    log_runtime_event(
                        LogLevel::Warn,
                        "pairing_code_enrollment_prepare_failed",
                        &[
                            ("peer", &peer.to_string()),
                            ("reason", &format!("{error:?}")),
                        ],
            ...
            if let Err(error) = context.code_pairing_sessions.prepare_enrollment(
                ...
                fail_code_pairing_join(
                    context,
                    &outbound.operation_id,
                    "pairing acceptance could not be recorded durably",
                )?;
                log_runtime_event(
                    LogLevel::Warn,
                    "pairing_code_enrollment_record_failed",
                    &[("peer", &peer.to_string()), ("reason", &error.to_string())],
                );
                return Ok(());
            }
            ...
            let remote_peer = match commit_pairing_runtime_enrollment(
                context.forwarder,
                context.membership,
                context.local_capabilities,
                prepared,
            ...
            if let Err(error) = context.code_pairing_sessions.complete_join(
                ...
                *response,
                ...
                log_runtime_event(
                    LogLevel::Error,
                    "pairing_code_join_finalize_failed",
                    &[("peer", &peer.to_string()), ("reason", &error.to_string())],
                );
                return Ok(());
            }
                ...
                .mark_enrollment_applied(&outbound.operation_id)
            {
                log_runtime_event(
                    LogLevel::Error,
                    "pairing_code_enrollment_mark_failed",
                    &[("peer", &peer.to_string()), ("reason", &error.to_string())],
                );
                return Ok(());
            }
        ...
        (OutboundCodeRequest::Poll(outbound), PairingCodeResponse::Rejected { reason }) => {
            ...
            if !matches!(
                ...
                fail_code_pairing_join(
                    context,
                    &outbound.operation_id,
                    format!("pairing approval poll rejected: {reason:?}"),
                )?;
            }
        }
        (pending, response) => {
            let operation_id = match &pending {
                OutboundCodeRequest::Hello(outbound) => outbound.operation_id.as_str(),
                OutboundCodeRequest::Submit(outbound) | OutboundCodeRequest::Poll(outbound) => {
                    outbound.operation_id.as_str()
                }
            }
            .to_owned();
            let expected = outbound_code_request_name(&pending);
            let actual = pairing_code_response_name(&response);
            release_outbound_code_request(context.code_pairing_sessions, &pending);
            if !matches!(&pending, OutboundCodeRequest::Hello(_)) {
                fail_code_pairing_join(
                    context,
                    &operation_id,
                    "pairing peer returned a response for the wrong protocol phase",
                )?;
            }
            log_runtime_event(
                LogLevel::Warn,
                "pairing_code_response_phase_mismatch",
                &[
                    ("peer", &peer.to_string()),
                    ("request_phase", expected),
                    ("response_kind", actual),
                ],
            );
        }
    }
...
fn release_outbound_code_request(
    sessions: &mut CodePairingSessions,
...
fn outbound_code_request_name(request: &OutboundCodeRequest) -> &'static str {
    match request {
        OutboundCodeRequest::Hello(_) => "hello",
        OutboundCodeRequest::Submit(_) => "submit",
        OutboundCodeRequest::Poll(_) => "poll",
    }
}

fn pairing_code_response_name(response: &PairingCodeResponse) -> &'static str {
    match response {
        PairingCodeResponse::Challenge { .. } => "challenge",
        PairingCodeResponse::Accepted { .. } => "accepted",
        PairingCodeResponse::Pending { .. } => "pending",
        PairingCodeResponse::Rejected { .. } => "rejected",
    }
}

fn fail_code_pairing_join(
    context: &mut SwarmEventContext<'_>,
    operation_id: &str,
    ...
    context
        .code_pairing_sessions
        .fail_join(operation_id, reason);
    if let Err(error) = persist_code_pairing_sessions(
        context.pairing_state_store,
        context.code_pairing_sessions,
        &context.local_capabilities.network_name,
    ) {
        log_pairing_persistence_failure("join_failure", &error);
    }
    Ok(())
}

fn send_pairing_code_unavailable(
    swarm: &mut Swarm<Behaviour>,
    channel: request_response::ResponseChannel<PairingCodeResponse>,
) -> Result<(), RunnerError> {
    send_pairing_code_response(
...
fn prepare_pairing_runtime_enrollment(
    forwarder: &Forwarder,
    offer: &PairingOffer,
    response: &PairingResponse,
    identity: &NodeIdentity,
    ...
    let remote_peer = if identity.peer_id == response.payload.inviter_peer {
        response
            .payload
            .joiner_peer
            .parse::<Libp2pPeerId>()
...
fn commit_pairing_runtime_enrollment(
    forwarder: &mut Forwarder,
    membership: &mut OverlayMembership,
    local_capabilities: &mut ControlCapabilities,
...
fn commit_pairing_runtime_enrollment_with(
    ...
    *local_capabilities = refreshed_local_capabilities(local_capabilities, forwarder);
    local_capabilities.membership_tag = prepared.membership_tag;
    log_runtime_event(
        LogLevel::Info,
        "pairing_runtime_enrollment_committed",
        ...
        if let Err(error) = execute(command) {
            for rollback in update.rollback_commands()[..applied].iter().rev() {
                if let Err(rollback_error) = execute(rollback) {
                    log_runtime_event(
                        LogLevel::Error,
                        "pairing_tun_rollback_failed",
                        &[
                            ("command", &rollback.to_string()),
                            ("reason", &format!("{rollback_error:?}")),
                        ],
                    );
                }
            }
...
fn execute_ip_command(command: &IpCommand) -> Result<(), RunnerError> {
    ...
    if status.success() {
        ...
        Err(RunnerError::TunRouteCommand {
...
fn promote_all_authenticated_overlay_connections(
    swarm: &mut Swarm<Behaviour>,
    context: &mut SwarmEventContext<'_>,
    peer: Libp2pPeerId,
    reason: &str,
    ...
    for connection_id in &connection_ids {
        promote_authenticated_overlay_connection(swarm, context, peer, *connection_id, reason);
    }
    if connection_ids.is_empty() && context.forwarder.is_configured_transport_peer(peer) {
        send_control_capabilities(
            swarm,
            context.forwarder,
            peer,
    ...
    log_runtime_event(
        LogLevel::Info,
        "authenticated_overlay_connections_promoted",
        &[
            ("peer", &peer.to_string()),
            ("connections", &connection_ids.len().to_string()),
            ("reason", reason),
        ],
    );
}
...
#[allow(clippy::too_many_arguments)]
#[cfg(test)]
fn pairing_response_for_request_with_grants(
    config: &Config,
    identity: &NodeIdentity,
    consumed_tokens: &mut HashSet<String>,
    transport_peer: Libp2pPeerId,
...
fn pairing_offer_and_response_for_request_with_grants(
    ...
    let inviter_route_grants = pairing_inviter_route_grants(config)?;
    let mut inviter_roles = vec![MembershipRole::OverlayMember];
...
fn pairing_inviter_route_grants(
    ...
    let mut grants = config.network.routes.clone();
...
fn pairing_route_grants_for_assignment(
    request: &PairingRequest,
    assigned_vpn_ip: Option<&str>,
    granted_routes: Option<Vec<RouteConfig>>,
) -> Result<Vec<RouteConfig>, crate::pairing::PairingError> {
    let mut route_grants =
        granted_routes.unwrap_or_else(|| request.payload.requested_routes.clone());
    for route in &route_grants {
        route.prefix()?;
    }
    let Some(requested_vpn_ip) = assigned_vpn_ip else {
...
impl LocalInterfaceNetwork {
    fn contains(self, address: IpAddr) -> bool {
        match (self.ip, self.netmask, address) {
            (IpAddr::V4(ip), IpAddr::V4(netmask), IpAddr::V4(address)) => {
                let mask = u32::from(netmask);
                u32::from(ip) & mask == u32::from(address) & mask
            }
            (IpAddr::V6(ip), IpAddr::V6(netmask), IpAddr::V6(address)) => {
                let mask = u128::from(netmask);
                u128::from(ip) & mask == u128::from(address) & mask
            }
            _ => false,
        }
    }
...
fn local_interface_networks(excluded_interface_name: &str) -> Vec<LocalInterfaceNetwork> {
    let Ok(interfaces) = if_addrs::get_if_addrs() else {
        return Vec::new();
    };
    interfaces
        .into_iter()
        .filter(|interface| {
            interface.name != excluded_interface_name
        ...
        .filter_map(|interface| match interface.addr {
            if_addrs::IfAddr::V4(address) => {
                let ip = IpAddr::V4(address.ip);
                direct_libp2p_address_ip_is_advertisable(ip).then_some(LocalInterfaceNetwork {
                    ip,
                    netmask: IpAddr::V4(address.netmask),
                })
            }
            if_addrs::IfAddr::V6(address) => {
                let ip = IpAddr::V6(address.ip);
                direct_libp2p_address_ip_is_advertisable(ip).then_some(LocalInterfaceNetwork {
                    ip,
        ...
        .fold(Vec::new(), |mut networks, network| {
...
fn private_ip_address(address: IpAddr) -> bool {
    match address {
...
fn packet_plane_session_needs_negotiation(
    ...
    advertised_endpoint.is_some()
        && (current_endpoint != advertised_endpoint || !has_healthy_path_kind(paths, peer, path))
}

fn has_healthy_path_kind(paths: &PathSet, peer: PeerId, path: PathKind) -> bool {
    paths
        .candidates_for(peer)
    ...
    match event {
        ...
        BehaviourEvent::Mdns(mdns::Event::Expired(peers)) if context.discovery.mdns => {
            for (peer, address) in peers {
                context
                    .code_pairing_sessions
                    .remove_lan_candidate(peer, &address);
                if context.forwarder.is_configured_transport_peer(peer)
                    && relayed_address_relay_peer(&address).is_none()
                {
                    log_runtime_event(
                        LogLevel::Info,
                        "configured_peer_mdns_direct_address_retained",
                        &[
                            ("peer", &peer.to_string()),
                            ("address", &address.to_string()),
                        ],
    ...
    if kademlia_query_result_key(result).is_some_and(kademlia_key_is_peer_address_record) {
        match result {
            kad::QueryResult::PutRecord(Ok(kad::PutRecordOk { key })) => {
                log_runtime_event(
                    LogLevel::Info,
                    "kademlia_peer_address_record_published",
                    &[("key", &format!("{key:?}"))],
                );
            }
...
fn learn_peer_address(
    ...
    if relayed_address_relay_peer(&address).is_some() {
        ...
        if !relayed_address_uses_public_relay_transport(&address) {
            metrics.record_discovered_address_rejected();
            log_runtime_event(
                LogLevel::Info,
                "discovered_relayed_address_rejected",
                &[
                    ("peer", &peer.to_string()),
                    ("address", &address.to_string()),
                    ("reason", "non_public_relay_transport"),
                    ("source", source.as_str()),
                ],
            );
            return;
...
fn relay_base_address_from_relayed_listen_address(address: &Multiaddr) -> Option<Multiaddr> {
...
impl From<CodePairingSessionError> for RunnerError {
    fn from(error: CodePairingSessionError) -> Self {
        Self::CodePairingSession(error)
    }
}

impl From<crate::pairing::PairingError> for RunnerError {
    fn from(error: crate::pairing::PairingError) -> Self {
        Self::Pairing(error)
    }
}
...
mod tests {
    ...
    fn test_pairing_state_path(name: &str) -> PathBuf {
        ...
        fs::create_dir_all(&directory).expect("pairing test directory");
    ...
    fn pairing_test_node(identity: &NodeIdentity) -> P2pNode {
        ...
        .expect("pairing test node")
    }

    #[test]
    fn tun_route_transaction_rolls_back_only_successful_steps() {
        let local = PeerId::from_bytes([1; 32]);
        let remote = PeerId::from_bytes([2; 32]);
        let current = TunRuntimeConfig {
            name: "pv0".to_owned(),
            mtu: 1280,
            addresses: crate::runtime::tun::TunAddresses::for_peer(local),
            additional_addresses: Vec::new(),
            routes: Vec::new(),
        };
        let next = TunRuntimeConfig {
            additional_addresses: vec![
                IpCidr::new(IpAddr::V4(Ipv4Addr::new(10, 42, 0, 1)), 32).expect("local address"),
            ],
            routes: vec![crate::route::Route {
                owner: remote,
                prefix: IpCidr::new(IpAddr::V4(Ipv4Addr::new(10, 42, 0, 2)), 32)
                    .expect("remote route"),
                metric: 0,
            }],
            ..current.clone()
    ...
    fn failed_runtime_commit_keeps_durable_prepared_enrollment() {
        ...
        persist_code_pairing_sessions(Some(&store), &sessions, "lab")
            .expect("persist write-ahead enrollment");
        ...
        .expect("restore prepared enrollment");
        assert_eq!(
            restored
    ...
    async fn inconsistent_prepared_enrollment_fails_before_runtime_changes() {
        ...
        assert!(matches!(
            result,
            Err(RunnerError::CodePairingSession(
                CodePairingSessionError::Conflict
            ))
        ));
    ...
    #[tokio::test]
    async fn prepared_inviter_enrollment_reconciles_after_expired_restart() {
        let (config, inviter, joiner, offer, request, response) = code_pairing_runtime_fixture();
        let operation_id = crate::runtime::pairing_sessions::fresh_pairing_operation_id();
        let now = Instant::now();
        let mut sessions = CodePairingSessions::new();
        let started = sessions
            .open_with_id(operation_id.clone(), "lab", 15, 1_000, now)
            .expect("open pairing");
        let joiner_peer = joiner.peer_id.parse().expect("joiner peer");
        let approval = PendingApproval::new(
            operation_id.clone(),
        ...
        let state_path = test_pairing_state_path("inviter-restart");
        let store = PairingStateStore::new(&state_path);
        persist_code_pairing_sessions(Some(&store), &sessions, "lab")
        ...
        .expect("restore expired operation");
        assert!(matches!(
            restored.open_status(&operation_id),
            Ok(PairingOpenStatus::Expired)
        ));
        ...
        .expect("reconcile prepared enrollment");

        assert!(forwarder.is_configured_transport_peer(joiner_peer));
        assert!(membership.allows(joiner_peer));
        assert!(matches!(
            restored.open_status(&operation_id),
            Ok(PairingOpenStatus::Completed)
        ));
        assert_eq!(
            restored
                .enrollment(&operation_id)
        ...
        let persisted = CodePairingSessions::restore_persisted(
            &store.load().expect("load applied").expect("applied bytes"),
            "lab",
            1_020,
            now + Duration::from_secs(20),
        )
        .expect("restore applied state");
        assert_eq!(
            persisted
                .enrollment(&operation_id)
                .expect("persisted enrollment")
                .state,
            PairingEnrollmentState::Applied
        );
        fs::remove_dir_all(state_path.parent().expect("state directory"))
    ...
    #[tokio::test]
    async fn prepared_joiner_enrollment_reconciles_after_expired_restart() {
        let (mut config, inviter, joiner, offer, request, response) =
            code_pairing_runtime_fixture();
        config.network.local_peer = joiner.peer_id.clone();
        config.network.private_key = Some(joiner.private_key.clone());
        ...
        let state_path = test_pairing_state_path("joiner-restart");
        let store = PairingStateStore::new(&state_path);
        persist_code_pairing_sessions(Some(&store), &sessions, "lab")
            .expect("persist prepared state");
        let mut restored = CodePairingSessions::restore_persisted(
    ...
    #[test]
    fn code_pairing_mutations_require_durable_daemon_state() {
        assert!(pair_rpc_requires_durable_state(&PairRpcRequest::PairOpen {
    ...
    #[test]
    fn file_pairing_handler_rejects_code_approval_offer() {
        let inviter = NodeIdentity::generate_ed25519().expect("inviter identity");
        let joiner = NodeIdentity::generate_ed25519().expect("joiner identity");
        let joiner_peer = joiner.peer_id.parse().expect("joiner peer");
        let config = config_with_peer(&inviter, joiner_peer);
        ...
        .expect_err("code offers require the approval handler");
        ...
        assert!(matches!(
            error,
            crate::pairing::PairingError::ApprovalRequired
        ));
    }

    #[test]
    fn code_pairing_approval_uses_explicit_address_and_route_grants() {
        let inviter = NodeIdentity::generate_ed25519().expect("inviter identity");
        let joiner = NodeIdentity::generate_ed25519().expect("joiner identity");
        let joiner_peer = joiner.peer_id.parse().expect("joiner peer");
        let config = config_with_peer(&inviter, joiner_peer);
        ...
        let response = pairing_response_for_request_with_grants(
            ...
            1_002,
            PairingAcceptanceMode::CodeApproval,
            Some("10.42.0.88".to_owned()),
            Some(vec![RouteConfig {
                prefix: "10.60.0.0/24".to_owned(),
                metric: 20,
            }]),
        )
        ...
        let joiner_record = response
            ...
            .member_records
            .iter()
            .find(|record| record.payload.member_peer == request.payload.joiner_peer)
    ...
    #[test]
    fn pair_rpc_status_exposes_approval_identity_without_pairing_secret() {
        let inviter = NodeIdentity::generate_ed25519().expect("inviter identity");
        let joiner = NodeIdentity::generate_ed25519().expect("joiner identity");
        let joiner_peer = joiner.peer_id.parse().expect("joiner peer");
        let config = config_with_peer(&inviter, joiner_peer);
        ...
        let status = pairing_rpc_status(&sessions, &started.operation_id, "lab", &inviter.peer_id)
            .expect("status");
        let candidate = status.candidate.as_ref().expect("candidate");

        assert_eq!(status.role, PairRpcRole::Inviter);
        assert_eq!(status.phase, PairRpcPhase::AwaitingApproval);
        assert_eq!(candidate.approval_id, approval_id);
        assert_eq!(candidate.peer_id, joiner.peer_id);
        assert_eq!(candidate.requested_vpn_ip.as_deref(), Some("10.42.0.2"));
        assert_eq!(
            candidate.public_key_fingerprint,
            URL_SAFE_NO_PAD.encode(Sha256::digest(request.payload.joiner_public_key.as_bytes()))
        );
    ...
    #[test]
    fn runtime_state_lines_include_relay_circuit_counters() {
        let metrics = RuntimeMetrics::default();
        metrics.record_relay_outbound_circuit_established();
        metrics.record_relay_inbound_circuit_established();

        let lines = runtime_state_summary_lines(RuntimeStateSummaryView {
            snapshot: &metrics.snapshot(crate::queue::QueueStats::default()),
            configured_peers: 0,
            validated_peers: 0,
            replay_windows: 0,
            packet_in_flight: PacketInFlightStats::default(),
            ...
            packet_plane_replay_windows_per_session: 1024,
        });

        assert_lines_contain(
            &lines,
            &[
                "relay_outbound_circuits_established 1",
                "relay_inbound_circuits_established 1",
            ],
        );
    }
    ...
    #[tokio::test]
    async fn kademlia_quiet_mode_still_admits_relay_infrastructure_candidates() {
        let local_identity = crate::identity::NodeIdentity::generate_ed25519().expect("identity");
        let configured = peer_id();
        let relay = peer_id();
        let config = config_with_peer(&local_identity, configured);
        ...
        let result = kad::QueryResult::GetClosestPeers(Ok(kad::GetClosestPeersOk {
            ...
            peers: vec![
                kad::PeerInfo {
                    peer_id: configured,
                    addrs: vec![configured_address],
                },
                kad::PeerInfo {
                    peer_id: relay,
                    addrs: vec![relay_address.clone()],
                },
            ],
        }));
        ...
        assert_eq!(
            infrastructure_peers.snapshot(&node.swarm),
            RelayInfrastructureSnapshot {
                peers: vec![RelayInfrastructurePeerSnapshot {
                    peer: relay,
    ...
    #[tokio::test]
    async fn authenticated_peer_records_reject_private_relay_transports() {
        let local_identity = crate::identity::NodeIdentity::generate_ed25519().expect("identity");
        let configured = peer_id();
        let relay = peer_id();
        let config = config_with_peer(&local_identity, configured);
    ...
    #[test]
    fn relayed_peer_publication_requires_public_relay_transport() {
        let relay = peer_id();
        let public = [
            format!("/ip4/8.8.8.8/tcp/4001/p2p/{relay}/p2p-circuit"),
            format!("/dns4/relay.example.net/tcp/4001/p2p/{relay}/p2p-circuit"),
            format!("/ip6/2606:4700:4700::1111/tcp/4001/p2p/{relay}/p2p-circuit"),
        ];
        for address in public {
            let address: Multiaddr = address.parse().expect("address");
            assert!(relayed_address_uses_public_relay_transport(&address));
        }
    ...
    #[test]
    fn recovery_discovery_queries_only_when_kademlia_enabled() {
        assert!(should_query_configured_peer_recovery_discovery(
            &DiscoveryConfig::default()
        ));
        assert!(!should_query_configured_peer_recovery_discovery(
            &DiscoveryConfig {
    ...
    #[test]
    fn public_discovery_backoff_suppresses_default_bootstrap_after_transport_failure() {
        let mut backoff = PublicDiscoveryBackoff::from_bootstrap_defaults(true);
        let public_peer = *backoff
            .bootstrap_peers
            .iter()
            .next()
            .expect("public bootstrap peer");
        let address: Multiaddr = format!("/dnsaddr/bootstrap.libp2p.io/p2p/{public_peer}")
            .parse()
            .expect("public address");
        let error = DialError::Transport(vec![(
            address,
            TransportError::Other(io::Error::other("failed to negotiate transport protocol")),
        )]);
        let now = Instant::now();
    ...
    #[test]
    fn public_discovery_backoff_does_not_extend_during_suppressed_burst() {
        let mut backoff = PublicDiscoveryBackoff::from_bootstrap_defaults(true);
        let public_peer = *backoff
            .bootstrap_peers
            .iter()
        ...
        let first_suppressed_until = backoff.suppressed_until;

        assert_eq!(
            backoff.record_outgoing_error(Some(public_peer), &error, now + Duration::from_secs(1)),
            None
        );

        assert_eq!(backoff.suppressed_until, first_suppressed_until);
    ...
    #[test]
    fn relay_base_address_strips_circuit_and_local_peer_from_listen_address() {
        let relay = peer_id();
        let local = peer_id();
        let listen: Multiaddr =
            format!("/ip4/127.0.0.1/tcp/4001/p2p/{relay}/p2p-circuit/p2p/{local}")
                .parse()
                .expect("listen address");
        let relay_base: Multiaddr = format!("/ip4/127.0.0.1/tcp/4001/p2p/{relay}")
            .parse()
            .expect("relay base address");

        assert_eq!(
            relay_base_address_from_relayed_listen_address(&listen),
            Some(relay_base)
        );
    }
    ...
    #[test]
    fn relay_ready_peer_targets_synthesize_configured_peer_relay_addresses() {
        let local = peer_id();
        let relay = peer_id();
        let peer = peer_id();
        let connected = peer_id();
    ...
    fn relay_readiness_allows_listen_address_before_reservation_acceptance() {
        ...
        assert!(readiness.relay_ready(relay));
        assert_eq!(
            readiness.ready_relay_addresses(relay),
    ...
    #[test]
    fn relay_addresses_prefer_local_network_before_unreachable_private_network() {
        let relay = peer_id();
        let local_network = LocalInterfaceNetwork {
            ip: IpAddr::V4(Ipv4Addr::new(192, 168, 51, 10)),
            netmask: IpAddr::V4(Ipv4Addr::new(255, 255, 255, 0)),
        };
        let unreachable_private: Multiaddr = format!("/ip4/192.168.52.254/tcp/4001/p2p/{relay}")
            .parse()
            .expect("unreachable private relay");
        let public: Multiaddr = format!("/ip4/8.8.8.8/tcp/4001/p2p/{relay}")
            .parse()
            .expect("public relay");
        let local: Multiaddr = format!("/ip4/192.168.51.254/tcp/4001/p2p/{relay}")
            .parse()
            .expect("local relay");
        let mut addresses = vec![
            (relay, unreachable_private.clone()),
            (relay, public),
            (relay, local.clone()),
        ];

        sort_relay_addresses_by_local_reachability(&mut addresses, &[local_network]);

        assert_eq!(addresses[0], (relay, local));
        assert_eq!(addresses[2], (relay, unreachable_private));
    }
    ...
    fn auto_relay_candidate_addresses_prefer_same_local_subnet() {
        ...
        let other_private_subnet: Multiaddr = format!("/ip4/192.168.52.254/tcp/4001/p2p/{peer}")
            .parse()
            .expect("other-subnet relay address");
    ...
    #[test]
    fn auto_relay_candidate_recording_keeps_best_ranked_address_for_peer() {
        let relay = peer_id();
        let same_subnet: Multiaddr = format!("/ip4/192.168.51.254/tcp/4001/p2p/{relay}")
            .parse()
            .expect("same-subnet relay address");
        let other_private_subnet: Multiaddr = format!("/ip4/192.168.1.3/tcp/4001/p2p/{relay}")
            .parse()
            .expect("other-subnet relay address");
        let public_address: Multiaddr = format!("/ip4/8.8.8.8/tcp/4001/p2p/{relay}")
            .parse()
            .expect("public relay address");
        let local_networks = [LocalInterfaceNetwork {
            ip: IpAddr::V4(Ipv4Addr::new(192, 168, 51, 1)),
        ...
        record_auto_relay_candidates(&mut state, &metrics, relay, ranked.into_iter().take(1));

        assert_eq!(
            state.next_reservation_targets(Instant::now()),
            vec![(relay, same_subnet)]
        );
    }

    #[test]
    fn connected_probe_endpoint_becomes_auto_relay_candidate_address() {
        let relay = peer_id();
        let dialer_address: Multiaddr = format!("/ip4/192.168.51.254/tcp/4001/p2p/{relay}")
            .parse()
            .expect("dialer address");
        let listener_send_back: Multiaddr = "/ip4/192.168.51.254/tcp/4001"
            .parse()
            .expect("listener send-back address");

        assert_eq!(
            connected_auto_relay_candidate_address(
                relay,
                &ConnectedPoint::Dialer {
                    address: dialer_address.clone(),
        ...
        assert_eq!(
            connected_auto_relay_candidate_address(
                ...
                &ConnectedPoint::Listener {
                    local_addr: "/ip4/192.168.51.1/tcp/4001".parse().expect("local addr"),
                    send_back_addr: listener_send_back,
                },
            ),
            Some(
                format!("/ip4/192.168.51.254/tcp/4001/p2p/{relay}")
                    .parse()
    ...
    fn connected_probe_endpoint_rejects_relayed_candidate_address() {
        ...
        let peer = peer_id();
        let relayed_address: Multiaddr =
            format!("/ip4/192.168.51.254/tcp/4001/p2p/{relay}/p2p-circuit/p2p/{peer}")
                .parse()
                .expect("relayed address");
    ...
    #[test]
    fn local_interface_network_matches_ipv4_and_ipv6_subnets() {
        let ipv4 = LocalInterfaceNetwork {
            ip: IpAddr::V4(Ipv4Addr::new(192, 168, 51, 10)),
            netmask: IpAddr::V4(Ipv4Addr::new(255, 255, 255, 0)),
        };
        let ipv6 = LocalInterfaceNetwork {
            ip: IpAddr::V6("2001:db8:abcd::1".parse().expect("ipv6 ip")),
            netmask: IpAddr::V6("ffff:ffff:ffff:ffff::".parse().expect("ipv6 netmask")),
        };

        assert!(ipv4.contains(IpAddr::V4(Ipv4Addr::new(192, 168, 51, 254))));
        assert!(!ipv4.contains(IpAddr::V4(Ipv4Addr::new(192, 168, 52, 254))));
        assert!(ipv6.contains(IpAddr::V6(
            "2001:db8:abcd::ffff".parse().expect("ipv6 address")
        )));
        assert!(!ipv6.contains(IpAddr::V6(
            "2001:db8:abce::1".parse().expect("ipv6 address")
        )));
    }

    #[test]
    fn auto_relay_state_attempts_candidates_regardless_of_autonat_reachability() {
        let relay = peer_id();
        let public_relay = peer_id();
        let address: Multiaddr = format!("/ip4/127.0.0.1/tcp/4001/p2p/{relay}")
            .parse()
            .expect("relay address");
        let public_address: Multiaddr = format!("/ip4/127.0.0.1/tcp/4002/p2p/{public_relay}")
            .parse()
            .expect("relay address");
        ...
        assert!(state.release_reservation_for_retry(relay));
        assert!(state.record_candidate(public_relay, public_address.clone()));
        state.record_reachability(AutoNatReachability::Public);

        assert_eq!(
            state.next_reservation_targets(now),
            vec![(relay, address), (public_relay, public_address)]
        );
    }

    #[test]
    fn auto_relay_state_discovers_candidates_even_when_autonat_reports_public() {
        let mut state = AutoRelayState::default();
    ...
    #[test]
    fn auto_relay_state_rotating_pending_candidate_waits_for_pending_attempt() {
        let relay = peer_id();
        ...
        assert!(
            state
                .next_reservation_targets(now + AUTO_RELAY_RESERVATION_PENDING_TIMEOUT)
                .is_empty()
        );
        assert_eq!(
            state.next_reservation_targets(
                now + AUTO_RELAY_RESERVATION_PENDING_TIMEOUT + Duration::from_secs(30)
            ),
    ...
    #[tokio::test]
    async fn kademlia_relay_infrastructure_rotates_existing_relay_address() {
        let mut node = build_node(&HostConfig {
            identity: crate::identity::NodeIdentity::generate_ed25519().expect("identity"),
            network_name: "lab".to_owned(),
            membership_tag: None,
        ...
        assert!(infrastructure_peers.insert(relay, stale_address.clone()));
        assert!(auto_relay.record_candidate(relay, stale_address));
        ...
        assert_eq!(infrastructure_peers.peers.get(&relay), Some(&fresh_address));
        assert_eq!(
            auto_relay.next_reservation_targets(now),
            vec![(relay, fresh_address)]
        );
        let snapshot = metrics.snapshot(crate::queue::QueueStats::default());
        assert_eq!(snapshot.auto_relay_infrastructure_candidates, 1);
    ...
    #[tokio::test]
    async fn kademlia_relay_infrastructure_respects_auto_relay_candidate_limit() {
        let mut node = build_node(&HostConfig {
            identity: crate::identity::NodeIdentity::generate_ed25519().expect("identity"),
            network_name: "lab".to_owned(),
            membership_tag: None,
        ...
        assert!(infrastructure_peers.contains(relay_a));
        assert!(!infrastructure_peers.contains(relay_b));
        assert_eq!(auto_relay.snapshot(Instant::now()).candidates, 1);
        assert_eq!(
            auto_relay.next_reservation_targets(Instant::now()),
            vec![(relay_a, address_a)]
        );
    ...
    #[test]
    fn relay_server_removes_unconfirmed_infrastructure_without_rejecting_client() {
        let peer = peer_id();
        let address: Multiaddr = format!("/ip4/127.0.0.1/tcp/4001/p2p/{peer}")
            .parse()
            .expect("address");
        ...
        assert!(remove_unconfirmed_infrastructure_peer(
            &mut infrastructure_peers,
            &mut auto_relay,
            peer,
            "missing_relay_hop",
    ...
    #[test]
    fn relay_server_external_address_filter_requires_specific_ip() {
        let concrete_v4: Multiaddr = "/ip4/192.168.41.254/tcp/4001".parse().expect("concrete v4");
