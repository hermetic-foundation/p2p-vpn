...
2. MTU handling deliberately rejects oversized packets, writes local
   packet-too-big feedback where possible, provides Linux route MSS hints, and
   learns lower selected-path ceilings from oversized outbound drops, but it
   does not implement overlay fragmentation or active probe-based PMTUD.
