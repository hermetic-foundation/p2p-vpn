...
| Discovery and public libp2p/IPFS bootstrap | partial | Private Kademlia, mDNS, configurable bootstrap peers, relay reservations, and an opt-in IPFS Kademlia protocol/bootstrap template exist. Public peers are documented as reachability infrastructure, not membership or route authority. mDNS and DHT namespace tests now verify discovery, capability exchange, packet-plane endpoint advertisement, signed packet-plane session negotiation, and real datagram packet forwarding after discovery. | `nix develop -c cargo test runtime::p2p::tests::build_node_accepts_ipfs_compatible_kademlia_protocol`; mDNS E2E: `nix run .#tun-e2e -- tun_namespace_ping_crosses_mdns_discovered_overlay -- --ignored --exact --nocapture`; DHT E2E: `nix run .#tun-e2e -- tun_namespace_ping_crosses_dht_discovered_overlay -- --ignored --exact --nocapture` |
