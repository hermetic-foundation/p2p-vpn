...
| Resource comparison | [Bounded sustained audit complete](sustained-resource-acceptance.md): ledger/queue ownership, Linux/Android sustained measurements, graceful teardown, final-code load comparison and pressure timer-capacity attribution. Release/physical-platform limits remain separate. |
...
| Packet stream ownership | [Stream regressions](packet-stream-ownership-review.md) include 1,500 TCP/QUIC cycles each. [Corrected S4](sustained-pressure-results.md#full-campaign-summary) passed 20 local TCP pressure/recovery rounds; its probe-owner regression distinguishes rejected-request metadata from admitted payloads. Allocation attribution and broader platform evidence remain open; historical failures are retained. |
...
| Resource measurement | [Sustained resource review complete](sustained-resource-acceptance.md): original criteria reconciled with workload, ownership, validation and cleanup evidence; unassigned-byte and platform limits are explicit |
