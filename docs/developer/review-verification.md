...
| Packet stream ownership | [Stream regressions](packet-stream-ownership-review.md) include 1,500 TCP/QUIC cycles each. The [sustained pressure investigation](sustained-pressure-results.md#deterministic-readiness-regression) adds current path health to the drain boundary after a deterministic fixture regression. Corrected sustained repetitions, allocation attribution and broader platform evidence remain open; historical failures are retained. |
